...
object FeatureEngineering {
   ...
   */
    //负责 One-hot 编码的转换器，通过它的 fit 函数完成指定特征的预处理，并利用 transform 函数将原始特征转换成 One-hot 特征
    //one-hot 将类别型数据转化为数值型，方便表征
    //将movieid转换为one-hot向量
  def oneHotEncoderExample(samples:DataFrame): Unit ={
      //samples样本集中的每一条数据代表一部电影的信息，其中movieId为电影id
      //新增一列movieIdNumber，用cast将string类型改为数值类型。onehotencoder必须输入数值型
  ...
  def main(args: Array[String]): Unit = {
    ...
    //设置Logger日志级别，ERROR level指出虽然发生错误事件，但仍然不影响系统的继续运行
